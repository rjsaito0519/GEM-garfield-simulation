...
Install ROOT first, since Garfield++'s build depends on it. Follow
ROOT's own build instructions
(\url{https://root.cern/install/build_from_source/}) for your platform;
in short, for a from-source build:
...
After installing, source ROOT's environment script before using it:
...
This project does \emph{not} rely on \texttt{thisroot.sh} being sourced
globally (e.g. from \texttt{.bashrc}) -- each script/build step sets the
paths it needs explicitly (see Section~\ref{sec:paths}), so that the
shell startup used for day-to-day work is not slowed down by always
loading a full ROOT environment. Set it up however is convenient for your
own workflow; just be consistent about which ROOT installation every
other tool below is pointed at.
...
Any Python 3 environment (conda, venv, or a plain \texttt{pip install
--user}) works; this project does not depend on a specific environment
manager. Install:
...
Section~3.3), pass \texttt{-DROOT\_DIR=...} explicitly as shown there.
...
\begin{itemize}
  ...
  \item \textbf{CMake resolving the wrong ROOT\_DIR}: force it explicitly
    if a fresh \texttt{cmake ..} picks the wrong one (Section~3.3).
