...
## 2026-09-24: 5x5 vs 7x7タイル収束性比較 — **5x5は収束しておらず、境界損失が透過率を大きく過小評価している**（issue #7 item 1、重要な発見）

production条件と同じ50イベント・narrow injection・Penning有効で、
7x7タイル(`triple_gem_field_v1.15x_n7`)でも同じ電子雪崩計算を実行し、
`analyze_plane_crossings.py`（後述の性能修正後）で解析、5x5と直接比較した。

| 指標 | 5x5 (n5) | 7x7 (n7) |
|---|---|---|
| 全avalanche electrons | 30,523 | 67,498 |
| GEM1-extracted cohort | 9054/30523 (29.7%) | 7368/67498 (10.9%) |
| GEM2ホール進入（cohort比） | 213/9054 (2.4%) | 1085/7368 (**14.7%**) |
| GEM2 bottom（cohort比） | 43/9054 (0.5%) | 227/7368 (**3.1%**) |
| GEM3 top（cohort比） | 1/9054 (0.0%) | 32/7368 (0.4%) |
| GEM3 bottom（cohort比、完全通過） | 0/9054 (0.0%) | **6/7368 (0.1%)** |
| 最終fateの"lateral boundary escape" (transfer gap 1) | **34.2%** | **上位リストに出てこない(<0.1%程度)** |

**結論: 5x5は明らかに収束していない。** 5x5ではGEM1-extracted cohortの
最終fateの34.2%がタイル境界からのラテラル脱出（"StatusLeftDriftArea
(lateral sensor boundary) in transfer gap 1"）だったが、7x7ではこの
損失チャネルが上位リストからほぼ消えた（境界に近い電子の割合が
大幅に減ったため）。この境界損失が減ったことで、GEM2ホール進入率が
2.4%→14.7%（約6倍）、GEM2 bottom到達が0.5%→3.1%（約6倍）と大きく
上昇し、**GEM3を完全に通過する電子が7x7で初めて複数件（6件）観測された**
（5x5では50イベント中ゼロ）。

これは「5x5で十分収束しているか確認する」というissue #7 item 1の
問いに対する直接的な答えで、**5x5は不十分、境界アーティファクトが
透過率を大きく過小評価している**ことを意味する。README.md「現在の
production condition」はn_cells=5を採用しているが、この結果を踏まえると
n_cells=7（またはさらに大きい値）への変更を検討すべき — ユーザーに
確認が必要な判断のため、まだREADME.mdは更新していない。

（7x7が本当に収束しているかは未確認。より大きいn_cells、例えば9x9等
での追試験が理想だが、7x7だけでもメッシュが542万ノード・ElmerSolverで
ILU2が整数オーバーフローする規模であり、計算コストが急増する点に注意
——上記のILU1回避策を使えば実行は可能）。

### 副産物: `analyze_plane_crossings.py`のメモリ不足バグを発見・修正

7x7データ（67,498トラック、2130万点）に対して最初に流したところ
`numpy._core._exceptions._ArrayMemoryError`で失敗した。原因は、
ユニークな(event,track)キーごとに毎回フルサイズの配列全体に対して
`(event==ev)&(track==tr)`という真偽値マスクを作り直し、しかも全キー分の
マスクを辞書にキャッシュして保持し続ける実装になっていたこと
（O(トラック数×全点数)の時間・メモリ）。`np.lexsort`による1回の
安定ソート+グループ境界検出に置き換え、各キーをソート済み配列への
軽量なスライスとして扱うよう修正した（安定ソートなので各トラック内の
記録順序は保持される。解析ロジック自体は一切変更していない）。
5x5データで修正前後の出力が完全に一致することを確認済み（コミット
`5c71698`）。
