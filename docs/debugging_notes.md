...
- 残る作業（未実施、優先度順）: (i) ~~単段GEM100モデルのtransfer field scan
  ...
  （GEM抽出効率自体は電場非依存、transfer gap侵入深さは電場依存性ありと
  判明したが単一セル境界アーティファクトと未分離。単段テストベッドの
  3x3タイル化が次の課題）、(ii) 孔taper形状の感度scanを同判定で再評価、
  ...
  アーティファクトを減らすため単段テストベッドも含め3x3/5x5タイル化を試す、
  (vi) 統計を100-200
