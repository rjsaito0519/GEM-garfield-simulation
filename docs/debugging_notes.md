...
- 電場の「向き」（仮説1）・「大きさ」（上記scan）・注入距離（仮説2）・注入方向
  ...
  3段スタック特有性は、いずれも原因ではないと切り分けられた。
- 残る有力候補: (a) `AvalancheMicroscopic`のtracking解像度（`SetCollisionSteps`等）が
  この急峻な電場勾配・狭い孔に対して粗すぎる可能性、(b) 孔内部の局所電場
  （中心軸プローブで確認した通り約38000-42000V/cm、外部transfer電場の2-10kV/cmより
  ずっと強い）が支配的なため、外側の条件を変えても孔内部の物理（真の電子吸収）が
  変わらないという、モデル上のバグではなく**現実的な物理的帰結である可能性**。
