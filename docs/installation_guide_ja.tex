...
かかります)場合、その環境を1つのsquashfsイメージにまとめて、
ノードローカルの\texttt{/tmp}配下に読み取り専用でマウントするのが
有効な対策の一つです。本プロジェクトの開発機自体がまさにこの方式を
使っています(そのマシン固有の実装詳細は、本リポジトリの外にある
\texttt{\textasciitilde/local/envfs\_README.md}を参照してください)
――これは純粋な性能最適化であり、完全に任意であり、本書の他の内容
とは独立しています: バッチ(LSF)ジョブは、実体としての(マウント
されていない)パスを常に見るため、これの影響を一切受けません。
