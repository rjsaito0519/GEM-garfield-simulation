...
Usage:
    ...
    visualization/output/<baseName>_z_profiles.png. Ne(z) is skipped (with
    a printed note) if no trajectory CSV exists yet for baseName -- run
...
def load_ne_profile(csv_path: str, n_bins: int = 200) -> tuple[np.ndarray, np.ndarray]:
    """Approximate Ne(z): the number of distinct (event,track) electrons
    (macros/export_avalanche_trajectories.cpp) whose recorded path passes
    through each z-bin, averaged over the number of distinct primary
    events in the file -- reads as "electrons present per primary event",
    not a raw total that would scale with however many events were run.
    This is a rough proxy (path points aren't evenly spaced in time or
    distance), not a proper time-resolved electron count."""
...
def main() -> None:
    ...
    out_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
        OUTPUT_DIR, f"{base_name}_z_profiles.png"
    )
    ...
    has_traj = os.path.exists(trajectories_path)
    ...
    if has_traj:
        ...
        print(f"No trajectory CSV at {trajectories_path} -- skipping Ne(z) "
